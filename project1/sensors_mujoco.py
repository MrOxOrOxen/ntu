...
class SensorModel:
    ...
    def add_imu_noise(self, ideal_accel, ideal_gyro, dt=IMU_DT):
        self.update_biases(dt)
        accel_noise = self.rng.normal(0, SIGMA_ACCEL, size=3)
        gyro_noise = self.rng.normal(0, SIGMA_GYRO, size=3)
        accel_measured = np.asarray(ideal_accel) + self.accel_bias + accel_noise
        gyro_measured = np.asarray(ideal_gyro) + self.gyro_bias + gyro_noise
        return accel_measured, gyro_measured